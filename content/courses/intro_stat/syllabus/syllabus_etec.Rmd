...
- Aulas : 3a, 14h-16h; e 5a, 14h-16h

- Instruções para o envio de e-mails: 
Por favor adicione ``[ETEC]'' ao 
...
- **Prova 1**: Ocorrerá no dia 07/07. 
Incluirá todos os tópicos vistos em aula.

- **Prova 2**: Ocorrerá no dia 11/08. 
Incluirá todos os tópicos vistos em aula,
com ênfase no material após a Prova 1.

- **Prova 3**: Ocorrerá no dia 27/09. 
...
`%>%` = magrittr::`%>%`
calendar_table = yaml::read_yaml("calendar_etec.yaml") %>% 
  do.call(rbind, .) %>% 
  as.data.frame() %>% 
  tidyr::unnest() %>%
  tibble::as_tibble() %>%
