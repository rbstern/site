...
- **Prova 1**: Ocorrerá no dia 06/07. 
Incluirá todos os tópicos vistos em aula.

- **Prova 2**: Ocorrerá no dia 10/08. 
Incluirá todos os tópicos vistos em aula,
...
`%>%` = magrittr::`%>%`
calendar_table = yaml::read_yaml("calendar_each.yaml") %>% 
  do.call(rbind, .) %>% 
  as.data.frame() %>% 
  tidyr::unnest() %>%
  tibble::as_tibble() %>%
