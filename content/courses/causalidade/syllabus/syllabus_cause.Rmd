...
- Aulas: 2a, 16h00-17h40; e 5a, 16h00-17h40

- Instruções para o envio de e-mails: 
Por favor adicione ``[MAE418]'' ao 
...
```{r echo = FALSE, message = FALSE}
library(magrittr)
keys = c("Participação (geral)", "Lista 1", "Lista 2", "Lista 3")
...
`%>%` = magrittr::`%>%`
calendar_table = yaml::read_yaml("calendar_cause.yaml") %>% 
  do.call(rbind, .) %>% 
  as.data.frame() %>% 
  tidyr::unnest() %>%
  tibble::as_tibble() %>%
